...
$show  = optional_param('show', 0, PARAM_INT);// Elo: Duy show
// Elo: Duy end
...
$PAGE->set_url('/mod/forum/post.php', array(
        ...
        'edit'  => $edit,
        'delete'=> $delete,
        'hide'=> $hide,// Elo: Duy show
        'show'=> $show,// Elo: Duy show
        'prune' => $prune,
        'name'  => $name,
        'confirm'=>$confirm,
        'groupid'=>$groupid,
...
if (!empty($forum)) {      // User is starting a new discussion in a forum
    ...
    echo $OUTPUT->footer();
    die;
}else if (!empty($hide)) {  // User is hiding a post // Elo: Duy 
    if (! $post = forum_get_post_full($hide)) {
        print_error('invalidpostid', 'forum');
    }
    if (! $discussion = $DB->get_record("forum_discussions", array("id" => $post->discussion))) {
        print_error('notpartofdiscussion', 'forum');
    ...
    if ( !(($post->userid == $USER->id && has_capability('mod/forum:hideownpost', $modcontext))
                || has_capability('mod/forum:hideanypost', $modcontext)) ) {
        print_error('cannothidepost', 'forum');
    }


    $replycount = forum_count_replies($post);

    if (!empty($confirm) && confirm_sesskey()) {    // User has confirmed the hide
        //check user capability to hide post.
        $timepassed = time() - $post->created;
        if (($timepassed > $CFG->maxeditingtime) && !has_capability('mod/forum:hideanypost', $modcontext)) {
            print_error("cannothidepost", "forum",
                        forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));
        }
        ...
        } else if ($replycount && !has_capability('mod/forum:hideanypost', $modcontext)) {
            print_error("couldnothidereplies", "forum",
                        forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));

        } else {
            if (! $post->parent) {  // post is a discussion topic as well, so delete discussion
                if ($forum->type == 'single') {
                    notice("Sorry, but you are not allowed to hide that discussion!",
                           forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));
                }

                redirect("view.php?f=$discussion->forum");

            } else if (forum_hide_post($post, has_capability('mod/forum:hideanypost', $modcontext),
                $course, $cm, $forum)) {
                ...
                print_error('errorwhilehide', 'forum');
            }
        }


    } else { // User just asked to hide something

        forum_set_return();
        $PAGE->navbar->add(get_string('hide', 'forum'));
        $PAGE->set_title($course->shortname);
        $PAGE->set_heading($course->fullname);
        ...
        if ($replycount) {
            ...
            echo $OUTPUT->header();
            echo $OUTPUT->heading(format_string($forum->name), 2);
            echo $OUTPUT->confirm(get_string("hidesureplural", "forum", $replycount+1),
                         "post.php?hide=$hide&confirm=$hide",
                         $CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'#p'.$post->id);
            ...
            echo $OUTPUT->header();
            echo $OUTPUT->heading(format_string($forum->name), 2);
            echo $OUTPUT->confirm(get_string("hidesure", "forum", $replycount),
                         "post.php?hide=$hide&confirm=$hide",
                         $CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'#p'.$post->id);
            forum_print_post($post, $discussion, $forum, $cm, $course, false, false, false);
        }
    ...
    echo $OUTPUT->footer();
    die;
} else if (!empty($show)) {  // User is showing a post // Elo: Duy show
    if (! $post = forum_get_post_full($show)) {
        print_error('invalidpostid', 'forum');
    }
    if (! $discussion = $DB->get_record("forum_discussions", array("id" => $post->discussion))) {
        print_error('notpartofdiscussion', 'forum');
    ...
    if ( !(($post->userid == $USER->id && has_capability('mod/forum:hideownpost', $modcontext))
                || has_capability('mod/forum:hideanypost', $modcontext)) ) {
        print_error('cannotshowpost', 'forum');
    }


    $replycount = forum_count_replies($post);

    if (!empty($confirm) && confirm_sesskey()) {    // User has confirmed the show
        //check user capability to show post.
        $timepassed = time() - $post->created;
        if (($timepassed > $CFG->maxeditingtime) && !has_capability('mod/forum:hideanypost', $modcontext)) {
            print_error("cannotshowpost", "forum",
                        forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));
        ...
        if ($post->totalscore) {
            notice(get_string('couldnotshowratings', 'rating'),
                   forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));

        } else if ($replycount && !has_capability('mod/forum:hideanypost', $modcontext)) {
            print_error("couldnotshowreplies", "forum",
                        forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));

        } else {
            if (! $post->parent) {  // post is a discussion topic as well, so show discussion
                if ($forum->type == 'single') {
                    notice("Sorry, but you are not allowed to show that discussion!",
                           forum_go_back_to(new moodle_url("/mod/forum/discuss.php", array('d' => $post->discussion))));
                }

                redirect("view.php?f=$discussion->forum");

            } else if (forum_show_post($post, has_capability('mod/forum:hideanypost', $modcontext),
                $course, $cm, $forum)) {
                ...
                print_error('errorwhileshow', 'forum');
            }
        }
    } else { // User just asked to show something

        forum_set_return();
        $PAGE->navbar->add(get_string('show', 'forum'));
        $PAGE->set_title($course->shortname);
        $PAGE->set_heading($course->fullname);
        ...
        if ($replycount) {
            ...
            echo $OUTPUT->header();
            echo $OUTPUT->heading(format_string($forum->name), 2);
            echo $OUTPUT->confirm(get_string("showsureplural", "forum", $replycount+1),
                         "post.php?show=$show&confirm=$show",
                         $CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'#p'.$post->id);
            ...
            echo $OUTPUT->header();
            echo $OUTPUT->heading(format_string($forum->name), 2);
            echo $OUTPUT->confirm(get_string("showsure", "forum", $replycount),
                         "post.php?show=$show&confirm=$show",
                         $CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'#p'.$post->id);
            forum_print_post($post, $discussion, $forum, $cm, $course, false, false, false);
        }
