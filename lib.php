...
function forum_get_user_grades($forum, $userid = 0) {
    ...
    $elo_course_create_time = $DB->get_record("course",array("id"=>$forum->course),'timecreated');
    $ratingoptions->elo_forum_rating_course_create_time = $elo_course_create_time->timecreated;
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (!isset($cm->cache->caps)) {
        ...
        $cm->cache->caps['mod/forum:hideownpost']    = has_capability('mod/forum:hideownpost', $modcontext);// Elo: Duy
        $cm->cache->caps['mod/forum:hideanypost']    = has_capability('mod/forum:hideanypost', $modcontext);// Elo: Duy
    ...
    if (!empty($post->deleted)) {
        ...
        if($post->deleted == 2){
            $output .= html_writer::start_tag('div', [
                ...
                'role' => 'region',
                'aria-label' => get_string('forumbodyhided', 'forum'),
            ]);
        ...
        if($post->deleted == 2){
            ...
            $output .= html_writer::tag('div', get_string('forumsubjecthided', 'forum'), [
                'class' => 'subject',
                'role' => 'header',
            ]);

            // Elo: Duy - show information
        ...
        $output .= html_writer::tag('div', '&nbsp;', ['class' => 'left side']); // Groups.
         ...
         if($post->deleted == 2){
            // Elo: Duy show
            $showbtn = "";
            if(has_capability('mod/forum:hideanypost', $modcontext))
            {
                $showhref = new moodle_url('/mod/forum/post.php', array('show'=>$post->id));
                $showbtn .= html_writer::tag('a', get_string('show', 'forum'), [
                    'class' => 'subject showbtn',
                    'role' => 'header',
                    'href' => $showhref
                ]);
    ...
    if (empty($str)) {
        ...
        $str->hide       = get_string('hide', 'forum');// Elo: Duy
        $str->show       = get_string('show', 'forum');// Elo: Duy
    ...
    // Elo: Duy hide
    if (($ownpost && $age < $CFG->maxeditingtime && $cm->cache->caps['mod/forum:hideownpost']) || $cm->cache->caps['mod/forum:hideanypost']) {
        $commands[] = array('url'=>new moodle_url('/mod/forum/post.php', array('hide'=>$post->id)), 'text'=>$str->hide);
    }
    // Elo: Duy end
    ...
    //Nhien show div start
    $canrate = has_capability('mod/forum:rate', $modcontext);
    if($canrate && !empty($post->rating)){
        $isnotrate = forum_is_null_rate($post, $discussion, $forum, $cm, $course);
        if($isnotrate == 'notrate'){
            $norate = get_string('discussionnorate', 'forum');
            $puspose ='menurating'.$post->id;
            $output .= '<div id="business' . $puspose . '" class= "colortext"><i class="fa fa-long-arrow-right"></i>' . $norate . '</div>';    
        }
    }
    // Nhien show div end    
    // Begin header row.
    ...
    $output .= html_writer::start_tag('div', array('class'=>'row side'));//Nhien them style
...
function forum_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
    ...
    if (has_capability('mod/forum:viewdiscussion', $modcontext)) {   // Show the column with replies
        ...
        if($post->elorates > -1)
        {
            echo '<td class="elorates">'; // Nhien elo rates 
            echo '<a href="'.$CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'">';
            echo  $post->elorates.'</a>';
            echo "</td>\n";
        }
...
function elo_forum_get_discussion_subscription_icon_preloaders() {
    $o = '';
    $o .= html_writer::span('&#9993;', 'preload-subscribe');
    $o .= html_writer::span('&nbsp;', 'preload-unsubscribe');
    return $o;
...
// Elo: Duy add hide post function
function forum_hide_post($post, $children, $course, $cm, $forum, $skipcompletion=false) {
    global $DB, $CFG, $USER;
    require_once($CFG->libdir.'/completionlib.php');
    ...
    if ($children !== 'ignore' && ($childposts = $DB->get_records('forum_posts', array('parent'=>$post->id)))) {
       if ($children) {
           foreach ($childposts as $childpost) {
                forum_hide_post($childpost, true, $course, $cm, $forum, $skipcompletion);
           }
       } else {
           return false;
       }
    ...
    $forum_posts_object = new stdClass();
    $forum_posts_object->id           = $post->id;
    $forum_posts_object->deleted = 2;
    if ($DB->update_record('forum_posts', $forum_posts_object)) {
...
// Elo: Duy add hide post function end
// Elo: Duy add show post function
function forum_show_post($post, $children, $course, $cm, $forum, $skipcompletion=false) {
    global $DB, $CFG, $USER;
    require_once($CFG->libdir.'/completionlib.php');
    ...
    if ($children !== 'ignore' && ($childposts = $DB->get_records('forum_posts', array('parent'=>$post->id)))) {
       if ($children) {
           foreach ($childposts as $childpost) {
                forum_show_post($childpost, true, $course, $cm, $forum, $skipcompletion);
           }
       } else {
           return false;
       }
...
// Elo: Duy add show post function end
...
// Nhien Check Forum is grading
    $eloforumgrading = 0;
    $graderecset = $DB->get_recordset_select('grade_items', 'courseid = ' . $course->id . 
            ' AND iteminstance = '. $forum->id , null,'','gradetype', 0,0);
    ...
    $graderecset->close();
    if (has_capability('mod/forum:rate', $context) && $eloforumgrading) {
        $eloforumgrading = 1;
    }
    else{
        $eloforumgrading = 0;
...
// Nhien Check Forum is grading
    if ($displayformat == 'header') {
        ...
        if (has_capability('mod/forum:viewdiscussion', $context)) {
            ...
            if ($eloforumgrading) {
                echo '<th class="header rates" scope="col">'.get_string('rates', 'forum').'</th>';
            }
        ...
        if ((!is_guest($context, $USER) && isloggedin()) && has_capability('mod/forum:viewdiscussion', $context)) {
            if (\mod_forum\subscriptions::is_subscribable($forum)) {
                ...
                echo elo_forum_get_discussion_subscription_icon_preloaders();
                echo '</th>';
            }
        }
        echo '</tr>';
    ...
    foreach ($discussions as $discussion) {
        ...
        $discussion->elorates = -1;
        if($eloforumgrading){
            $discussion->elorates = elo_get_count_post_notrated_for_top_discussion($course, $forum,$cm,$discussion,0);
        }
        ...
        $elosumreplies += $discussion->replies;
        if($eloforumgrading) $elosumelorates += $discussion->elorates;
        $elosumunread +=$discussion->unread;
    }
    
    switch ($displayformat) {
        case 'header':
            echo '<td class="sumelorates">' . get_string('total') . '</td>';
            echo '<td class="sumelorates"></td>';                
            if ($groupmode > 0) echo '<td class="sumelorates"></td>';
            echo '<td class="sumelorates"><span >'. $elosumreplies.'</span></td>';
            if($eloforumgrading) echo '<td class="sumelorates"><span>'. $elosumelorates.'</span></td>';
            echo '<td class="sumelorates"><span>'. $elosumunread.'</span></td>';
            echo '<td class="sumelorates"></td>';
            echo '<td class="sumelorates"></td>';                                  
        break;
    }        
...
function forum_print_discussion($course, $cm, $forum, $discussion, $post, $mode, $canreply=NULL, $canrate=false) {
    ...
    //Nhien start
    if($canrate && !empty($post->rating)){
            $chocount = forum_count_not_rate_posts_nested($course, $cm, $forum, $discussion, $post);
            $teachercount = forum_count_notrate_answer_for_teacher($course, $cm, $forum, $discussion, $post);
            echo '<div class="card my-3">' . 
                    '<div class="card-header">'
                    . '<h4 class="mb-0 paneltitle-rating text-center">'
                    . '<a role="button" data-toggle="collapse" data-target="#collapseRating" aria-expanded="true" aria-controls="collapseRating">Thảo luận chưa được đánh giá</a>'
                    . '</h4>'
                    . '</div>' ;
            echo '<div id="collapseRating" class="panel-collapse collapse show" role="tabpanel">'
            . '<div class="row">'
                    . '<div class="col-12 col-lg-6 card-body">'
                    . '<div class="pl-lg-2 font-weight-bold">'.get_string('eventdiscussionnorate', 'forum').': <span class="colortext">'. $chocount->countnorate .'</div>'
                        . '<div class="pl-lg-2">'. $chocount->noratelink . '</div>'
                    . '</div>'
                    . '<div class="col-12 col-lg-6">
                        <div class="row">
                            <div class="col-12 col-lg-12 card-body">
                                <div class="font-weight-bold">Trả lời cho người khởi tạo('.$teachercount->countnorateteacher.' thành viên)</div>
...
//Nhien start
function forum_count_notrate_answer_for_teacher($course, $cm, $forum, $discussion, $parent){
    $list_norate_teacher->noratelinkteacher = '';
    $list_norate_teacher->countnorateteacher = 0;
    if (!empty($parent->children)) {
        foreach ($parent->children as $post) {
            $israte = forum_is_null_rate($post, $discussion, $forum, $cm, $course); 
            if ($israte == 'notrate') {
                $list_norate_teacher->countnorateteacher ++;
                $discussionlinkteacher = new moodle_url('/mod/forum/discuss.php', array('d'=>$parent->discussion),'p' . $post->id);
                /*if (isset($post->rating->rating)) {
                    $point = '(Điểm:'.$post->rating->rating.')';
                }
                else {*/
                    $point = '<span class="text-danger">(Chưa chấm điểm)</span>';
                //}
                /*if ($post->rating->rating == 0 && isset($post->rating->rating)) {
                    $point = '<span class="text-muted">('.get_string("notrating", "rating").')</span>';
                }*/
                $list_norate_teacher->noratelinkteacher .= html_writer::link($discussionlinkteacher, $list_norate_teacher->countnorateteacher . '. ' . $post->lastname . ' ' . $post->firstname . ' : ' . $post->subject);
                $list_norate_teacher->noratelinkteacher.= $point;
                $list_norate_teacher->noratelinkteacher .= '</br>'; 
            }    
        }
    }
    return $list_norate_teacher;
}

function forum_count_not_rate_posts_nested($course, $cm, $forum, $discussion, $parent, $number = 0 ){
    global $elo_forum_count_not_rate_posts_nested;
    if($number == 0){
        $elo_forum_count_not_rate_posts_nested = 0;
    }
    //$countnorate = 0;// Chi Tinh Con So Dem Post Chua Cham
    // $parent: thuc ra cai nay la no' khong phai cha no'
    $list_norate->countnorate = 0;
    $list_norate->noratelink = '';
    $israte = forum_is_null_rate($parent, $discussion, $forum, $cm, $course);
    if($israte == 'notrate'){
        $elo_forum_count_not_rate_posts_nested ++;
        $list_norate->countnorate ++;
        $discussionlink = new moodle_url('/mod/forum/discuss.php', array('d'=>$discussion->id),'p' . $parent->id);
        $list_norate->noratelink .= html_writer::link($discussionlink, $elo_forum_count_not_rate_posts_nested . '. ' . $parent->lastname . ' ' . $parent->firstname . ' : ' . $parent->subject);
        $list_norate->noratelink .= '</br>'; 
    }
...
//            $israte = forum_is_null_rate($post, $discussion, $forum, $cm, $course);
//            if($israte =='notrate'){
//                $countnorate = $countnorate + 1;
//            }
            $countnotrates = forum_count_not_rate_posts_nested($course, $cm, $forum, $discussion, $post, 1);
            $list_norate->countnorate = $list_norate->countnorate + $countnotrates->countnorate;
            $list_norate->noratelink .= $countnotrates->noratelink; 
        }
    }
    return $list_norate;
}
...
function elo_get_count_post_notrated_array($courseid,$forumid,$teacher_userid){
       global $CFG,$DB;
       require_once($CFG->dirroot.'/rating/lib.php');
       ...
       if ($displaymode == FORUM_MODE_FLATNEWEST) {
           $sort = "p.created DESC";
       } else {
           $sort = "p.created ASC";
       }
       ...
       foreach ($discussions as $key => $discussion){
           $discussionchild = $DB->get_record('forum_discussions', array('id' => $discussion->discussion), '*', MUST_EXIST);
           $posts = forum_get_all_discussion_posts($discussion->discussion,$sort,$forumtracked);
           ...
           if ($forum->assessed != RATING_AGGREGATE_NONE ) {
               ...
               $ratingoptions->aggregate = $forum->assessed;//the aggregation method
               $ratingoptions->scaleid = $forum->scale;
               $ratingoptions->userid = $teacher_userid;
               if ($forum->type == 'single' or !$discussionchild->id) {
                   $ratingoptions->returnurl = "$CFG->wwwroot/mod/forum/view.php?id=$cm->id";
               } else {
                   $ratingoptions->returnurl = "$CFG->wwwroot/mod/forum/discuss.php?d=$discussion->id";
               }
           ...
           $chocounttruture = forum_count_not_rate_posts_nested($course, $cm, $forum, $discussionchild,$post);
           $elotopic->eloname = $forum->name;
           $elotopic->elonum = $chocounttruture->countnorate;
           $chocount[] = $elotopic;
       }
       return $chocount;
}
//nhien elo
function elo_get_submissionsneedgradingcount($id) {
    global $CFG;
...
function elo_get_assigns_topics($course,$userid) {
	global $DB;
        $eloassigns = array();
        $elotopics = array();    
        ...
        foreach ($modules as $module) {
            ...
            if($module->modname =='forum'){
                $numrorate = elo_get_count_post_notrated($course->id,$module->instance,0);
                    
                $test = count($elotopics);
                $elotopics[$test]->eloname = 'forum';//'Chưa chấm điểm'.$module->name;
                $elotopics[$test]->id = $module->id;//'Chưa chấm điểm'.$module->name;
                $elotopics[$test]->shortname = $module->name;
                $elotopics[$test]->elonum = $numrorate;
                $elotopics[$test]->eloviewname = get_string('notratesforum', 'forum') . ($test + 1);
                $elotopics[$test]->eloorder = $test+1;                         
            }
            if($module->modname =='assign'){
                $submissionsneedgradingcount = elo_get_submissionsneedgradingcount($module->id);
                $testassign = count($eloassigns);
                $eloassigns[$testassign]->eloname = 'assign';
                $eloassigns[$testassign]->id = $module->id;
                $eloassigns[$testassign]->shortname = $module->name;
                $eloassigns[$testassign]->elonum = $submissionsneedgradingcount;
                $eloassigns[$testassign]->eloviewname = get_string('notratesassginment', 'forum') . ($testassign + 1);  
                $eloassigns[$testassign]->eloorder = $testassign + 1;
            }                    
        }
        $eloactivities = $eloassigns;
        foreach($elotopics as $topic){
            $eloactivities[] = clone $topic;
        }         
        return $eloactivities;//$htmlyear . $htmlcoures;
    }
function elo_get_count_post_notrated($courseid,$forumid,$teacher_userid){
    global $CFG,$DB;
    ...
    foreach ($discussions as $key => $discussion){
        ...
        $chocounttruture = forum_count_not_rate_posts_nested($course, $cm, $forum, $discussionchild,$post);
       $chocount += $chocounttruture->countnorate;
    }
    return $chocount;
}
function elo_get_count_post_notrated_for_top_discussion($course,$forum,$cm,$discussion,$teacher_userid){
    global $CFG,$DB;
    require_once($CFG->dirroot.'/rating/lib.php');
//    $course = $DB->get_record('course', array('id' => $courseid), '*', MUST_EXIST);
//   $forum = $DB->get_record('forum', array('id' => $forumid), '*', MUST_EXIST);
